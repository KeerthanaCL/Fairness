...
const FairnessMetricsDashboard = ({ analysisId }) => {
  const [selectedAttribute, setSelectedAttribute] = useState('gender'); // Default to gender
  const [availableAttributes, setAvailableAttributes] = useState([]);

  // Debug: Log the analysisId
  console.log('FairnessMetricsDashboard received analysisId:', analysisId);

  // Fetch available sensitive attributes
  useEffect(() => {
    const fetchSensitiveAttributes = async () => {
      if (analysisId) {
        try {
          const sensitiveFeatures = await FairnessAPIService.getSensitiveFeatures(analysisId);
          console.log('Fetched sensitive features:', sensitiveFeatures);
          
          if (sensitiveFeatures?.detectedFeatures) {
            const attributes = sensitiveFeatures.detectedFeatures.map(feature => feature.name);
            setAvailableAttributes(attributes);
            
            // Set first available attribute as default if current selection is not available
            if (attributes.length > 0 && !attributes.includes(selectedAttribute)) {
              setSelectedAttribute(attributes[0]);
            }
          }
        } catch (error) {
          console.error('Failed to fetch sensitive attributes:', error);
          // Fallback to default attributes
          setAvailableAttributes(Object.values(SENSITIVE_ATTRIBUTES));
        }
      } else {
        // No analysis ID, use default attributes
        setAvailableAttributes(Object.values(SENSITIVE_ATTRIBUTES));
      }
    };

    fetchSensitiveAttributes();
  }, [analysisId, selectedAttribute]);

  // Custom hook for fairness metrics with proper attribute-based caching
  const useFairnessMetricsWithAttribute = (analysisId, attribute) => {
    ...
    useEffect(() => {
      if (!analysisId || !attribute) return;
    ...
    }, [analysisId, attribute, refreshTrigger]);
  ...
  const {
    ...
    error,
    refresh
  } = useFairnessMetricsWithAttribute(analysisId, selectedAttribute);
  ...
  const attributeData = useMockData 
    ...
    : apiData?.[selectedAttribute];

  console.log('Using mock data:', useMockData);
  console.log('Attribute data:', attributeData);
  console.log('Full API data structure:', apiData);
  console.log('Accessing apiData[' + selectedAttribute + ']:', apiData?.[selectedAttribute]);
